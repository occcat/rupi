...
逐子系统的上游对照、有意偏离与未移植清单见 [`docs/UPSTREAM.md`](docs/UPSTREAM.md)；
三个历史分支的评审见 [`BRANCH_COMPARISON.md`](BRANCH_COMPARISON.md)。
...
| sessions are trees（branch/rewind/summary） | `SessionTree::branch_from` / `rewind_to` / `prompt_history` 压缩窗口 + `AgentLoop::maybe_compress`（按 token：`reserveTokens`/`keepRecentTokens`，默认 16384/20000；`--compress-threshold/--compress-keep` 或 settings `compaction`，`RUPI_COMPRESSION_OVERRIDES` 按 `provider/model` 覆盖，对标上游 `compaction.modelOverrides`；溢出报错强制压实 + 同 turn 重发（`MAX_OVERFLOW_RECOVERIES=2` 封顶）） |
...
| 会话持久化 | 每轮落盘 `sessions.db`（每个节点：`content` 纯文本供检索 + `blocks` 完整消息 JSON），`sessions` / `session-show` / `session-search`，`--session <id\|jsonl>` / `-r` / `--fork` / `--resume <id>` / `--continue` 续聊、`--no-session` 临时、`--name` 命名；`/new` `/session` `/export` JSONL（Pi v3）或 HTML、`/import`、`/fork`（当前路径）、`/clone`（全树）；`/tree` + `/goto <短id>` |
...
| coding-agent CLI + TUI | `rupi-cli`（无子命令默认 TUI，`chat` 仍是 REPL；`run`/`-p` 读管道 stdin；位置参数 `@file`；`--session`/`-r`/`--fork`；`--no-context-files`、`-e`/`--skill`/`--no-extensions`/`--no-skills`/`--no-builtin-tools`；`--tools/--exclude-tools/--no-tools`、`--system-prompt/--append-system-prompt`；配置 `~/.rupi/settings.json` + 项目 `.rupi/settings.json`（model/thinking/compaction/tools/theme）以及 `SYSTEM.md`/`APPEND_SYSTEM.md`；状态栏 `↑↓ tokens / context% / $`） |
...
# 配置：~/.rupi/settings.json 与项目 .rupi/settings.json（后覆盖前）
...
# 非交互 + JSONL 事件流（对标 pi --mode json；每行一个 AgentEvent，末行 run_result）
...
- `[core]` 分层：条目以 `[core]` 开头即固定注入每个 session 的提示；一旦存在任何 `[core]` 条目，未标记条目成为 extended 层，只在 `memory_search`（FTS 镜像 + 文件子串兜底）时召回。无标记时全量注入，向后兼容。
