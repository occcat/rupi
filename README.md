...
| 会话持久化 | 每轮落盘 `sessions.db`（每个节点：`content` 纯文本供检索 + `blocks` 完整消息 JSON），`sessions` / `session-show` / `session-search`，`--resume <id>` / `--continue` 续聊、`--no-session` 临时、`--name` 命名；`/export` JSONL（Pi v3）或 HTML、`/import`、`/fork`（当前路径）、`/clone`（全树）；`/tree` + `/goto <短id>` |
...
| coding-agent CLI + TUI | `rupi-cli`（`chat` REPL + `run` + `tui`；`--tools/--exclude-tools/--no-tools`、`--system-prompt/--append-system-prompt`；配置 `~/.rupi/settings.json` + 项目 `.rupi/settings.json`（model/thinking/compaction/tools/theme）以及 `SYSTEM.md`/`APPEND_SYSTEM.md`；状态栏 `↑↓ tokens / context% / $`） |
