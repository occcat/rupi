...
| coding-agent CLI + TUI | `rupi-cli`（`rupi` 二进制；`chat` REPL（provider 出错只报错回滚本轮，不退出）+ `run` 非交互（`--approve/--no-approve`，`--review/--review-apply` 后台沉淀落盘，`--json` 输出 JSONL 事件流对标 `pi --mode json`）+ `tui` ratatui 全屏界面：流式渲染/滚动/review 行；TUI 内建命令与 REPL 同语义（`/rewind/plan/thinking/model`，`reload` 明确提示 REPL 专属）+ `/` 开头 Tab 补全，弹窗展示候选；三端均显示每轮 usage） |
