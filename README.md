...
- **Hermes 风格记忆**（`MEMORY.md` + `USER.md` 冻结快照、`[core]` 分层、FTS5 会话检索）
...
| 无内置记忆 | Hermes：冻结快照 + `memory` 工具 + SQLite FTS5 |
...
3. **记忆**：会话开始时把 USER/MEMORY 打进 prompt 后冻结（保 prefix cache）。当轮 `memory` 写入立刻落盘，但要到下一会话才进入 prompt。`[core]` 条目始终注入，其余走 `search`。
...
6. **权限**：cwd sandbox（默认拒绝越出工作区的 read/write/edit）；bash 可对破坏性命令 Ask/Deny。
7. **子 agent**：`run_subagent` 用受限 ToolSet 跑嵌套 loop，isolated 模式只把摘要交回父 agent。
